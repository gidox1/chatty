...
export const Card = () => {
  ...
  const getUnreadMsgCount = (msgs: SingleMessage[]): number => msgs.filter((m) => m.status === MessageStatus.UNREAD).length;
          ...
          <Box display="flex" h="15%" w="full"  background="gray.bg" borderRadius="5px" cursor="pointer" key={key}>
            {
              ...
              key === 0 && (
                <Box w="1%" borderRadius="5px 0 0 5px" background="green.chat" h="100%"></Box>
              )
            }
            <Box display="flex" flexDirection="column" w="full" h="full" p="1% 0 0 2%" >
              <Box w="full" h="65%" display="flex" flexFlow="row">
                ...
                <Box pl="5%" w="60%">
                  <Text fontSize="2xl">{info.username}</Text>
              ...
              </Box>
                {
                  info.messages.length > 0 && (
                    <Box w="full" h="full" display="flex" flexFlow="row" justifyContent="space-between">
                      <Text fontSize="12.5px" p="1% 0 0 6%">{
                      ...
                      <Box h="80%" w="7%" borderRadius="50%" display="flex" pos="relative" justifyContent="center" background="green.bg" mr=".5rem">
                        <Text color="white" textAlign="center" m="auto" pb="1px" pos="absolute">{getUnreadMsgCount(info.messages)}</Text>
                      </Box>
                    </Box>
                  )
                }
